...
function test_HiGHS_custom_options(::Any, ::Any)
    ...
    @test MOI.get(model, MOI.RawOptimizerAttribute("message_level")) == 4
    MOI.set(model, MOI.RawOptimizerAttribute("message_level"), 1)
    @test MOI.get(model, MOI.RawOptimizerAttribute("message_level")) == 1
    ...
    @test_throws MOI.UnsupportedAttribute MOI.get(model, MOI.RawOptimizerAttribute("wrong_param"))
...
function test_option_unknown_option(model, ::Any)
    ...
    @test_throws err MOI.set(model, MOI.RawOptimizerAttribute("write_solution_to_file"), 1)
    @test_throws err MOI.set(model, MOI.RawOptimizerAttribute("simplex_strategy"), "on")
...
function runtests()
    config = Dict(
        "simplex" => MOI.Test.Config(basis = true),
        "ipm" => MOI.Test.Config(basis = true, infeas_certificates = false),
    )
